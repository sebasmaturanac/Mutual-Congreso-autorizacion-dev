...
const DashboardVendedor = ({ history }) => {
  ...
  const handleRevisionUpdate = (personaToUpdate) =>
    setPersonasTable((personasTable) => {
      const existePersona = personasTable.some(
        ({ _id }) => _id === personaToUpdate._id,
      );
      if (!existePersona) return personasTable;
  ...
  const handleNuevaPersona = (personaToAdd) => {
    ...
    if (personaToAdd?.pedidos[0]?.vendedor.includes(alias)) {
      setPersonasTable((personasTable) => {
        const copyTable = personasTable.map((persona) => ({ ...persona }));
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'pedidos',
      render: (pedidos) => pedidos && moment(pedidos[0]?.createdAt).format('DD/MM/YYYY'),
    },
    {
      title: 'Estado último pedido ',
      dataIndex: 'pedidos',
      render: (pedidos) => {
        const label = pedidos && `${pedidos[0]?.estado}`;
        return label;
      },
    },
    {
    ...
    <>
      <div className="mt-4 mx-auto">
        <ToolbarTable
          ...
          placeholder="Buscar por nombre, apellido, DNI, banco y estado del ultimo pedido"
          disabled={loading}
        />
        <Table
          columns={columns}
